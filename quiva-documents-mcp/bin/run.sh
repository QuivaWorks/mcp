#!/bin/sh
# Launcher for the Quiva Documents (file-generator) MCP server.
#
# 1. Loads quiva-documents-mcp/.env (already-exported vars take precedence).
...
if [ -z "$NODE_BIN" ]; then
  echo "[quiva-documents-mcp] No Node >= 18 found. Set QUIVA_NODE to a Node 18+ binary in $ENV_FILE." >&2
  exit 1
fi
