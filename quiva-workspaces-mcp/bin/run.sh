#!/bin/sh
# Launcher for the Quiva Workspaces MCP server.
#
# 1. Loads quiva-workspaces-mcp/.env (already-exported vars take precedence).
...
if [ -z "$NODE_BIN" ]; then
  echo "[quiva-workspaces-mcp] No Node >= 18 found. Set QUIVA_NODE to a Node 18+ binary in $ENV_FILE." >&2
  exit 1
fi
